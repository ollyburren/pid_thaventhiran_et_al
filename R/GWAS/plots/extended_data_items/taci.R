...
meta.all <- fread("__REDACTED__/analysis/pid/GWAS/AbDef.maf05.opr98.chr1-23.assoc.logistic.plink")


taci.chr <- 17
#taci.start <- 16695860  16827000
taci.start <- 16827000
taci.end <- 17022299

meta <- meta.all[CHR==taci.chr & between(BP,taci.start,taci.end),][,region:=1]

r.gr <- with(meta,GRanges(seqnames=Rle(CHR),ranges=IRanges(start=BP,width=1L),mlp=-log10(P),pid=paste(CHR,BP,sep=':')))
## grab index.snp <-
...
ld.cmd <- sprintf(
    ...
    BCF_TOOLS,taci.chr,bcf_maf,taci.chr,taci.start,taci.end)
...
dummy <- rep(NA,nrow(dt)) %>% as.numeric
dt[,c('group.1','group.4','group.5'):=list(dummy,dummy,dummy)]
...
gene <- BiomartGeneRegionTrack(
    ...
    stackHeight=0.2, filters=list(with_ox_refseq_mrna=T,chromosome_name=taci.chr,
    #start=start(query.region),end=end(query.region),biotype='protein_coding',hgnc_symbol=list('BCL6','LPP')),
    start=taci.start-1e5,end=taci.end+1e5,biotype='protein_coding'),
...
tracks$interaction <- InteractionTrack(name="pcHi-C",interactions,chromosome="chr17")
...
pdf("~/tmp/taci.pdf",useDingbats=FALSE)
plotTracks(tracks, background.title = "white",fontcolor.title="black",col.border.title="black",cex.title = 1,from=16810000,to=pend,sizes=c(0.5,2,1,0.5,0.5,0.5,0.5,0.5))
dev.off()
