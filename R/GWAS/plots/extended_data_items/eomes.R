...
## TODO - MAKE GENE NAME FONTS BIGGER
## ADD GWAS CATALOGUE HITS
## DO A BLOW OUT TO HI-LIGHT EOMES
...
}) %>% rbindlist %>% with(.,GRanges(seqnames=Rle(chr),ranges=IRanges(start=pos,width=1L),disease=disease,id=id)) %>% subsetByOverlaps(.,plot.region.gr)
...
tracks$interaction <- InteractionTrack(name="pcHi-C",interactions,chromosome="chr3")
...
stop()
pdf("~/tmp/eomes.pdf",useDingbats=FALSE)
plotTracks(tracks, background.title = "white",fontcolor.title="black",col.border.title="black",cex.title = 1,from=pstart-5e4,to=pend,sizes=c(0.5,2,1,0.5,0.5,0.5,0.5,0.5))
dev.off()
...
zstart <- start(r.gr[!is.na(r.gr$group.7),])-2e4
#zend <- start(r.gr[!is.na(r.gr$group.7),])-1e4
zend <- start(r.gr[!is.na(r.gr$group.7),])+250
...
tracks$gene <- gene
tracks$gwas <- AnnotationTrack(name="GWAS",imb.gr,stacking="dense",col="black",fill="black",group=imb.gr$disease, just.group="above", groupAnnotation = "group",col.line="white")
pdf("~/tmp/eomes_blowup.pdf",useDingbats=FALSE)
plotTracks(tracks, background.title = "white",fontcolor.title="black",col.border.title="black",cex.title = 1,from=zstart,to=zend,sizes=c(0.3,0.3,1))
dev.off()
