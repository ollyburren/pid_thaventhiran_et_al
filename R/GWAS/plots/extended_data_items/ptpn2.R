...
regions <- meta[,list(min.p=min(overall.P)),by='region'][min.p<1e-6,]$region
...
dt[,group.5:=rep(NA,length(r.gr)) %>% as.numeric]
...
tracks <- list()
tracks$gene <- gene
tracks$meta <- DataTrack(name="Meta -log10(P)",r.gr,type=c("p"),groups=groups,col=c('grey',ldColGrad,'purple'),cex=1.5,legend=FALSE)
...
tracks$interaction <- InteractionTrack(name="pcHi-C",interactions,chromosome="chr18")
...
pdf("~/tmp/ptpn2.pdf")
plotTracks(tracks, background.title = "black",cex.title = 1.1,from=pstart-9e3,to=pend,cex.title=0.8)
dev.off()
